...
const SECTIONS = {
  ...
  viralHooks: {
    sys: c => `You are a viral short-form content creator who has studied thousands of viral reels and shorts. You write for PERFORMERS, not writers. Your hooks make people physically stop scrolling. Plain text only. ${c.stageNote} CRITICAL: Respond entirely in ${c.language}.`,
  ...
  script: {
    sys: c => `You are a viral short-form content creator — NOT a scriptwriter, NOT a journalist. You write for ${c.platform} reels and shorts that stop scrolls and hold attention till the last second. You think like a performer, not a writer. Plain text only. ${c.stageNote} CRITICAL: Respond entirely in ${c.language}.`,
    usr: c => `${c.base}\n\nRESEARCH INTEL (use specific details — this is your source material):\n${c.research}\n\nWrite a viral ${c.shortForm} script for a "${c.niche}" creator about this trend.\n\nBefore writing, identify:\n- RELATABLE TRUTH: what do "${c.niche}" people actually feel about this trend?\n- MISUNDERSTANDING: what does everyone get wrong?\n- EMOTIONAL TRIGGER: ego / confusion / love / irritation / shock — pick one and build around it\n\nThen write the script using VIRAL STRUCTURE:\n\nHOOK (0-2 sec): [CUT] One confusing/weird/unexpected line. Max 7 words. Don't explain it.\n\nOPEN LOOP (2-8 sec): [ZOOM] Don't explain the hook. Create more curiosity. 2-3 fast lines, each under 10 words. Broken sentences okay.\n\nESCALATION (8-25 sec): [CUT] [CUT] 2-3 micro-moments of tension. No long storytelling. Each line earns the next. Use Hinglish if natural for the niche. Interruptions (...) and pauses are good.\n\nTWIST (25-35 sec): [PAUSE] Unexpected but relatable revelation. NOT what they predicted. Specific to "${c.niche}" life.\n\nAFTERSHOCK (35-45 sec): Either "Wait... am I wrong?" energy OR "This happens to everyone right?" energy. End on comment bait.\n\nRULES:\n- No "Hey guys"\n- No "So basically"\n- No "In the end"\n- No long paragraphs\n- No explaining everything — leave gaps\n- Every line must feel clippable\n- Add [CUT] [ZOOM] [PAUSE] where editing should happen\n- Use broken sentences, interruptions (...), real reactions\n- Must feel like a real person, not AI\n\nAfter the main script, also write:\n5 TONE VARIANTS (just the hook + first 2 lines each):\n- FUNNY version:\n- SARCASTIC version:\n- DRAMATIC version:\n- ABSURD version:\n- ULTRA-RELATABLE version:\n\nFINAL CHECK: If this sounds clean or AI-written, rewrite it. It should feel like something Tanmay Bhatt would react to or a creator would actually record on first take.`,
  },
  visualIdeas: {
