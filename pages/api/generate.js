...
const SECTIONS = {
  ...
  usr: c => `${c.base}\n\nWrite 2 ready-to-paste captions using the RETENTION FRAMEWORK — tease the payoff, don't give it away:\nCAPTION 1: Short punchy (30-50 words) — establish the problem or curiosity, promise something surprising at the end + ${c.hashStyle}.\nCAPTION 2: Storytelling style (80-100 words) — open with a conflict, build tension mid-way, hint at a shocking twist or revelation at the end. Raw and genuine tone, no salesy language + region-specific hashtags for ${c.region}.`,
  },
  script: {
  sys: c => `You are an elite ${c.platform} scriptwriter. Plain text. Max 230 words. ${c.stageNote} CRITICAL: Your entire response must be written in ${c.language} only.`,
  usr: c => `${c.base}\n\nWrite a complete script for a ${c.shortForm} using the PSEUDO RETENTION FRAMEWORK:\n[HOOK 0:00-0:05] — Promise a payoff at the end ("stay till the end for X"). Establish the problem immediately.\n[CONFLICT 0:05-0:20] — Introduce a major conflict or tension. Make them feel "what's going to happen?"\n[MIDDLE 0:20-0:45] — Work towards the solution but keep key information hidden. Fresh angle, never-heard-before perspective. Raw and genuine — no selling, just someone who clearly enjoys this topic.\n[TENSION 0:45-0:55] — Keep tension alive. Viewer still can't predict the ending.\n[SHOCKING END 0:55-0:60] — Deliver the promised payoff with a surprising/shocking revelation or twist.\nConversational, natural to say aloud. Optimised for ${c.platform}.`,
  },
  visualIdeas: {
